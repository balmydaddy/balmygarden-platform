...
## ⚠️ 1-3-2. 재빌드해도 여전히 "키 미설정"이 뜬다면

이건 배포 대상 착오가 아니다. **실측 확인 결과**: 재빌드된 배포(작업 브랜치, 올바른 빌드)가 요청을 정상적으로 받았는데도 여전히 "키 미설정" 오류를 던졌다. 즉 코드도, 배포 대상도 문제가 아니고 **Vercel에 저장된 환경변수 자체가 이 빌드에 보이지 않는다**는 뜻이다. 아래 순서로 확인한다.

1. **Environment 체크박스** — Settings → Environment Variables에서 `NAVER_CLIENT_ID`(또는 NCP 키) 행을 펼쳐 **Preview**가 체크돼 있는지 확인한다. Production만 체크했다면 우리 작업 브랜치(Preview로 빌드됨)에는 애초에 전달되지 않는다.
2. **변수명 오타** — `NAVER_CLIENT_ID` / `NAVER_CLIENT_SECRET` 정확히 이 철자인지 확인한다. 둘 중 하나만 등록돼 있어도 코드가 그 경로를 아예 안 탄다(`resolveAuth()`가 두 값이 모두 있어야 채택).
3. **값 앞뒤 공백** — 붙여넣기 시 공백이나 줄바꿈이 같이 들어갔는지 값 칸을 다시 열어 확인한다.
4. **저장이 실제로 됐는지** — Save 버튼을 누른 뒤 목록에 다시 나타나는지 확인한다(새로고침 후 재확인).
5. 위 4개를 확인해도 안 되면, 등록한 값 자체가 잘못됐을 가능성이 크다 — 개발자센터에서 Client ID/Secret을 다시 복사해 새로 붙여넣는다.

**여기까지 확인한 값을 알려주면(스크린샷도 가능) 다음 진단으로 넘어간다.** Vercel MCP 도구로는 저장된 환경변수 값 자체를 읽을 수 없어(보안상 비공개) 여기서부터는 CEO 쪽 화면 확인이 필요하다.
