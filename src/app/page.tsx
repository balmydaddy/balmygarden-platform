...
const LS_ONBOARD = "receipt_onboarded";
...
function calcStats(items: Item[]): Record<string,ItemStat> {
  const map: Record<string,{name:string;category:Category;records:{date:string;store:string;price:number;total:number}[];storeMin:Record<string,number>}> = {};
  items.forEach(item => {
...
function OnboardingScreen({ onDone }: { onDone: () => void }) {
  ...
  const steps = [
    { icon:"📄", title:"영수증 사진 업로드", desc:"마트·편의점·온라인 영수증 사진을 업로드하세요. 여러 장을 한 번에 올릴 수 있습니다." },
    { icon:"🤖", title:"AI 자동 분석", desc:"AI가 영수증에서 품목·가격·구매처를 자동으로 추출합니다. 분석은 보통 3~5초 걸립니다." },
    { icon:"📊", title:"최저가 재구매 타이밍", desc:"같은 제품의 구매 이력을 비교해 지금이 재구매 적기인지 알려드립니다." },
    { icon:"🔔", title:"목표가 알림", desc:"원하는 가격 이하로 구매했을 때 이메일 알림을 받을 수 있습니다." },
    { icon:"☁️", title:"클라우드 동기화", desc:"로그인하면 모든 기기에서 구매 이력을 공유할 수 있습니다. 가족과 함께 사용해보세요!" },
  ];
  const cur = steps[step];
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:20000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#fff", borderRadius:16, padding:32, maxWidth:400, width:"100%", textAlign:"center" }}>
        ...
        <div style={{ display:"flex", justifyContent:"center", gap:6, marginBottom:20 }}>
          {steps.map((_,i)=>(
            <div key={i} style={{ width:8, height:8, borderRadius:"50%", background:i===step?"#3B6D11":"#ddd" }}/>
          ))}
        </div>
        <h2 style={{ fontSize:20, fontWeight:700, marginBottom:10 }}>{cur.title}</h2>
        <p style={{ fontSize:14, color:"#666", lineHeight:1.6, marginBottom:24 }}>{cur.desc}</p>
        <div style={{ display:"flex", gap:8 }}>
          {step > 0 && (
            <button onClick={()=>setStep(s=>s-1)} style={{ flex:1, padding:"10px", borderRadius:8, border:"1px solid #ddd", background:"#fff", cursor:"pointer", fontSize:14 }}>이전</button>
          )}
          {step < steps.length-1 ? (
            <button onClick={()=>setStep(s=>s+1)} style={{ flex:1, padding:"10px", borderRadius:8, border:"none", background:"#3B6D11", color:"#fff", cursor:"pointer", fontSize:14, fontWeight:600 }}>다음</button>
          ) : (
            <button onClick={onDone} style={{ flex:1, padding:"10px", borderRadius:8, border:"none", background:"#3B6D11", color:"#fff", cursor:"pointer", fontSize:14, fontWeight:600 }}>시작하기 🚀</button>
          )}
        </div>
  ...
  return (
    <div style={{ ...card, borderLeft:`3px solid ${cls==="best"?"#3B6D11":cls==="near"?"#378ADD":"#D85A30"}` }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
        ...
        <button onClick={()=>onSetAlert(stat.name)} style={{
        ...
        }}>🔔 {currentAlert?`${currentAlert.toLocaleString()}원`:"알림 설정"}</button>
      </div>
      ...
      <div style={{ fontSize:11, color:"#666", marginBottom:8 }}>최저가 구매처: <strong style={{ color:"#222" }}>{stat.bestStore}</strong> ({Math.round(stat.bestStorePrice).toLocaleString()}원)</div>
...
function ToastBar({ toasts }: { toasts:Toast[] }) {
  ...
  return (
    <div style={{ position:"fixed", bottom:24, right:16, display:"flex", flexDirection:"column", gap:8, zIndex:9999, maxWidth:"calc(100vw - 32px)" }}>
      {toasts.map(t=>(
        <div key={t.id} style={{
...
function NotifyModal({ notify, setNotify, onClose, onSave, stats }: {
  notify:NotifySettings; setNotify:(n:NotifySettings)=>void;
  onClose:()=>void; onSave:()=>void; stats:Record<string,ItemStat>;
}) {
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }} onClick={onClose}>
      <div style={{ background:"#fff", borderRadius:12, padding:24, maxWidth:500, width:"100%", maxHeight:"80vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
              ...
              {Object.values(stats).map(s=>{
                ...
                return (
                  <div key={s.name} style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 8px", background:"#f8f8f8", borderRadius:6 }}>
                    ...
                    <input type="number" value={alert?.targetPrice??""} onChange={e=>{
                      const v = Number(e.target.value);
                      if (v>0) updateAlert(s.name,v); else removeAlert(s.name);
                    }} placeholder="목표가" style={{ width:80, padding:"4px 6px", borderRadius:4, border:"1px solid #ccc", fontSize:12 }}/>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
          ...
          <button onClick={()=>{ setNotify({email,alerts}); onSave(); }} style={{ padding:"8px 16px", borderRadius:8, border:"none", background:"#3B6D11", color:"#fff", cursor:"pointer", fontSize:13, fontWeight:600 }}>저장</button>
        </div>
      </div>
    </div>
  );
...
export default function Home() {
    ...
    supabase.from("items").select("*").eq("user_id", user.id).order("created_at", { ascending: false })
      .then(({ data }) => {
        if (data && data.length > 0) {
          const cloud: Item[] = data.map((r: Record<string,unknown>) => ({
          ...
          setAllItems(cloud);
          toast(`클라우드에서 ${cloud.length}건 불러왔습니다`, "info");
        }
      });
  },[user]);
  ...
  const handleBarcodeScan = useCallback(async (barcode:string) => {
    ...
    toast("바코드 조회 중...", "info");
    try {
      const resp = await fetch("/api/barcode", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({barcode}) });
      const json = await resp.json();
      if (json.success) {
        const newItem: Item = { name:json.data.name, qty:1, unit_price:0, total:0, store:"수동 등록", date:new Date().toISOString().slice(0,10), _src:`barcode_${barcode}`, category:"기타" };
  ...
  const sendAlert = useCallback(async (matched:{name:string;target:number;current:number;store:string;date:string}[]) => {
    ...
    try {
      const resp = await fetch("/api/notify", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({email:notify.email,items:matched}) });
      const json = await resp.json();
      if (resp.ok&&json.success) toast(`${matched.length}건 목표가 알림 이메일 발송!`,"ok");
      else toast("알림 발송 실패: "+(json.error||""),"err");
    } catch(e) { console.error(e); toast("알림 발송 오류","err"); }
  },[notify.email]);
  ...
  const analyzeAll = useCallback(async ()=>{
    ...
    await Promise.all(pending.map(async entry=>{
      ...
      try {
        ...
        const resp = await fetch("/api/ocr",{ method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({image:b64,mediaType:file.type}) });
        const json = await resp.json();
        ...
        results[entry.id] = {
          ...
          items:(items||[]).map((it:Omit<Item,"store"|"_src">)=>({
            ...it, store:store||"알 수 없음", date:it.date||rd||"unknown", _src:entry.name, category:it.category||"기타",
          })),
        };
        ok++;
      } catch(e) { results[entry.id]={status:"error",items:[]}; err++; console.error(e); }
    }));
    ...
    if (user && newItems.length > 0) {
      const rows = newItems.map(item=>({
        user_id:user.id, family_code:null,
        name:item.name, qty:item.qty, unit_price:item.unit_price, total:item.total,
        store:item.store, date:item.date, src:item._src, category:item.category,
      }));
      const { error } = await supabase.from("items").insert(rows);
      if (!error) toast("클라우드 저장 완료!","ok");
      else console.error("Supabase 저장 오류:", error.message, error.code);
    }

    // 가격 알림 체크
    const matched = newItems.map(item=>{
  ...
  },[files, user, notify.alerts, sendAlert]);
  ...
  const deleteItem = async (idx:number) => {
    if (user) {
      ...
      await supabase.from("items").delete().eq("user_id",user.id).eq("name",item.name).eq("date",item.date).eq("store",item.store).limit(1);
    }
  ...
  const byName = Object.entries(allItems.reduce<Record<string,number>>((a,i)=>{ a[i.name]=(a[i.name]||0)+n(i.total); return a; },{})).sort((a,b)=>b[1]-a[1]).slice(0,10);
  const byStore = Object.entries(allItems.reduce<Record<string,number>>((a,i)=>{ a[i.store]=(a[i.store]||0)+n(i.total); return a; },{})).sort((a,b)=>b[1]-a[1]);
  const trendName = trendItem??Object.keys(stats)[0];
  const trendRecs = trendName?(stats[trendName]?.records??[]).filter(r=>r.date&&r.date!=="unknown"&&r.price>0).sort((a,b)=>a.date>b.date?1:-1):[];
  ...
  const trendData = Array.from(new Set(trendRecs.map(r=>r.date))).sort().map(date=>{
    ...
    trendStores.forEach(st=>{ const rec=trendRecs.find(r=>r.date===date&&r.store===st); if (rec) obj[st]=Math.round(rec.price); });
    return obj;
  });
  const histItems = allItems.filter(i=>{
    const kw=histSearch.toLowerCase();
    return (!kw||i.name.toLowerCase().includes(kw)||i.store.toLowerCase().includes(kw))&&(!histStore||i.store===histStore)&&(histCat==="전체"||i.category===histCat);
  }).sort((a,b)=>{
  ...
  const setAlertForItem = () => setShowNotify(true);
  ...
  return (
    <div style={{ maxWidth:960, margin:"0 auto", padding:"1rem" }}>
      ...
      {showNotify && (
        <NotifyModal notify={notify} setNotify={setNotify}
      ...
      <div style={{ marginBottom:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
          <div>
            <h1 style={{ fontSize:20, fontWeight:700, margin:0 }}>🛒 스마트 재구매 대시보드</h1>
            <p style={{ fontSize:12, color:"#888", marginTop:2 }}>영수증 → AI 분석 → 최저가 재구매 타이밍</p>
          </div>
          <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
            ...
            <button onClick={()=>setShowScanner(true)} style={{ fontSize:12, padding:"6px 12px", borderRadius:8, border:"1px solid #ddd", background:"#fff", cursor:"pointer", fontWeight:600 }}>📷 스캔</button>
            <button onClick={()=>setShowNotify(true)} style={{ fontSize:12, padding:"6px 12px", borderRadius:8, border:"1px solid "+(notify.alerts.length>0?"#3B6D11":"#ddd"), background:notify.alerts.length>0?"#EAF3DE":"#fff", cursor:"pointer", fontWeight:600, color:notify.alerts.length>0?"#3B6D11":"#222" }}>
              🔔{notify.alerts.length>0?` (${notify.alerts.length})`:""}
            </button>
            {hasDash&&(
              <>
                <button onClick={()=>exportCSV(allItems)} style={{ fontSize:12, padding:"6px 12px", borderRadius:8, border:"1px solid #ddd", background:"#fff", cursor:"pointer", fontWeight:600 }}>⬇ CSV</button>
                <button onClick={clearAll} style={{ fontSize:12, padding:"6px 12px", borderRadius:8, border:"1px solid #fcc", background:"#fff9f9", cursor:"pointer", color:"#c00", fontWeight:600 }}>🗑</button>
              </>
            )}
            <button onClick={()=>setShowOnboard(true)} style={{ fontSize:11, padding:"5px 8px", borderRadius:8, border:"1px solid #ddd", background:"#fff", cursor:"pointer", color:"#888" }}>❓</button>
          </div>
        </div>
      </div>
      ...
      {/* 업로드 영역 */}
      <div
        style={{ border:"1.5px dashed #ccc", borderRadius:12, padding:"1.5rem 1rem", textAlign:"center", cursor:"pointer", background:drag?"#f5f5f5":"#fff", marginBottom:8 }}
      ...
      >
        ...
        <strong style={{ fontSize:14 }}>영수증 사진 업로드</strong>
        <p style={{ fontSize:12, color:"#888", marginTop:4 }}>클릭 또는 드래그 · JPG / PNG / WEBP · 다량 가능</p>
      </div>
      <input ref={fileRef} type="file" accept="image/*" multiple style={{ display:"none" }} onChange={e=>addFiles(Array.from(e.target.files??[]))}/>
      ...
      {hasDash&&(
        <>
          {/* 탭 - 모바일 스크롤 */}
          <div style={{ display:"flex", gap:4, marginTop:20, borderBottom:"1px solid #eee", paddingBottom:8, overflowX:"auto", WebkitOverflowScrolling:"touch" as React.CSSProperties["WebkitOverflowScrolling"] }}>
            {TABS.map(t=><button key={t} style={{...navBtn(tab===t), whiteSpace:"nowrap"}} onClick={()=>setTab(t)}>{LABELS[t]}</button>)}
          </div>
            ...
            <>
              ...
              <div style={{ marginTop:16 }}>
                ...
                <div style={card}>
                  <ResponsiveContainer width="100%" height={260}>
                    <PieChart>
                      <Pie data={byCat} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90}
                        label={({name,percent}:{name?:string;percent?:number})=>name&&percent!==undefined&&percent>0.04?`${name} ${Math.round(percent*100)}%`:""}>
              ...
              <div style={{ marginTop:16 }}>
                ...
                <div style={{ ...card, overflowX:"auto" }}>
                  <ResponsiveContainer width="100%" height={Math.max(180,byCat.length*32+60)}>
                    <BarChart data={byCat} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/>
                      <XAxis type="number" tick={{fontSize:10}} tickFormatter={v=>v.toLocaleString()}/>
                      <YAxis type="category" dataKey="name" tick={{fontSize:10}} width={80}/>
                      <Tooltip formatter={fmt}/>
                      <Bar dataKey="value" radius={[0,4,4,0]}>{byCat.map(c=><Cell key={c.name} fill={CAT_COLORS[c.name]}/>)}</Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
            ...
            <div style={{ marginTop:16 }}>
              ...
              <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginBottom:12, overflowX:"auto" }}>
                {Object.keys(stats).map(name=>(
                  <button key={name} onClick={()=>setTrendItem(name)} style={{ fontSize:11, padding:"4px 10px", borderRadius:8, border:"1px solid #ddd", cursor:"pointer", background:trendName===name?"#f0f0f0":"#fff", fontWeight:trendName===name?600:400, whiteSpace:"nowrap" }}>{name}</button>
                ))}
              </div>
              <div style={card}>
                {trendData.length>0?(
                  <ResponsiveContainer width="100%" height={240}>
                    <LineChart data={trendData}>
                      ...
                      <Tooltip formatter={fmt}/>
                      <Legend/>
                      {trendMin>0&&<ReferenceLine y={trendMin} stroke="#3B6D11" strokeDasharray="4 3" label={{value:"최저가",fontSize:10,fill:"#3B6D11"}}/>}
                      {trendStores.map((st,i)=><Line key={st} type="monotone" dataKey={st} stroke={COLORS[i%COLORS.length]} strokeWidth={2} dot={{r:4}} connectNulls/>)}
                    </LineChart>
                  </ResponsiveContainer>
                ):(
              ...
              <div style={{ marginTop:16 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:8 }}>
                  {Object.entries(storeMap).sort((a,b)=>b[1].total-a[1].total).map(([nm,d])=>(
                    <div key={nm} style={{ background:"#f8f8f8", border:"0.5px solid #e5e5e5", borderRadius:10, padding:"10px 12px" }}>
                      ...
                      {([["총 지출",Math.round(d.total).toLocaleString()+"원"],["구매 횟수",d.cnt+"회"],["취급 품목",d.items.length+"종"]] as [string,string][]).map(([l,v])=>(
                        <div key={l} style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"#666", marginTop:2 }}>
          ...
          {tab==="history"&&(
            <div style={{ marginTop:16 }}>
              <div style={{ display:"flex", gap:6, marginBottom:10, flexWrap:"wrap", alignItems:"center" }}>
                <input style={{ flex:1, minWidth:100, padding:"6px 10px", borderRadius:8, border:"0.5px solid #ccc", fontSize:12 }}
                  placeholder="품목·구매처 검색..." value={histSearch} onChange={e=>setHistSearch(e.target.value)}/>
                <select style={{ padding:"5px 8px", borderRadius:8, border:"0.5px solid #ccc", fontSize:11 }} value={histCat} onChange={e=>setHistCat(e.target.value as Category|"전체")}>
                  <option value="전체">전체 카테고리</option>
                  {CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}
                </select>
                <select style={{ padding:"5px 8px", borderRadius:8, border:"0.5px solid #ccc", fontSize:11 }} value={histStore} onChange={e=>setHistStore(e.target.value)}>
                  <option value="">전체 구매처</option>
                  {Array.from(new Set(allItems.map(i=>i.store))).map(s=><option key={s} value={s}>{s}</option>)}
                </select>
                <select style={{ padding:"5px 8px", borderRadius:8, border:"0.5px solid #ccc", fontSize:11 }} value={histSort} onChange={e=>setHistSort(e.target.value)}>
                  <option value="date_desc">최신순</option>
                  <option value="date_asc">오래된순</option>
                  <option value="price_asc">단가 낮은순</option>
                  <option value="price_desc">단가 높은순</option>
                </select>
                <span style={{ fontSize:11, color:"#888" }}>{histItems.length}건</span>
              </div>
                    ...
                    {histItems.length===0?(
                      ...
                      return (
                        <tr key={idx} style={{ background:isMin?"#f0faf0":undefined }}>
                          <td style={{ padding:"6px 8px", borderBottom:"1px solid #f3f3f3", whiteSpace:"nowrap" }}>{item.date||"-"}</td>
                          <td style={{ padding:"6px 8px", borderBottom:"1px solid #f3f3f3" }}>
                            {item.name||"-"}
                            {isMin&&<span style={{ fontSize:9, background:"#EAF3DE", color:"#3B6D11", padding:"1px 5px", borderRadius:999, fontWeight:700, marginLeft:4 }}>최저가</span>}
                          </td>
                          <td style={{ padding:"6px 8px", borderBottom:"1px solid #f3f3f3" }}>
                            <span style={{ fontSize:9, padding:"2px 6px", borderRadius:999, fontWeight:600, background:CAT_COLORS[item.category]+"22", color:CAT_COLORS[item.category] }}>{item.category}</span>
                          </td>
                          <td style={{ padding:"6px 8px", borderBottom:"1px solid #f3f3f3" }}>
                            <span style={{ fontSize:10, background:"#f0f0f0", padding:"2px 6px", borderRadius:999 }}>{item.store}</span>
                          </td>
                          <td style={{ padding:"6px 8px", borderBottom:"1px solid #f3f3f3" }}>{n(item.qty)||1}</td>
                          <td style={{ padding:"6px 8px", borderBottom:"1px solid #f3f3f3", textAlign:"right" }}>{Math.round(up).toLocaleString()}</td>
                          <td style={{ padding:"6px 8px", borderBottom:"1px solid #f3f3f3", textAlign:"right" }}>{Math.round(n(item.total)).toLocaleString()}</td>
                          <td style={{ padding:"6px 8px", borderBottom:"1px solid #f3f3f3" }}>
